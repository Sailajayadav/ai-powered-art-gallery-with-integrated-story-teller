...
def search():
    ...
    if request.method == "POST":
        ...
        BASE_DIR = os.path.dirname(os.path.abspath(__file__))
        excel_path = os.path.join(BASE_DIR, 'static', 'data', 'artworks.xlsx')
...
def analyze_image_with_gemini(image_path):
    ...
    try:
        ...
        or
        UNSAFE: If the image contains any inappropriate content
        ...
        for file in files:
            if file and allowed_file(file.filename):
                ...
                if is_safe:
                    ...
                    # Remove unsafe image
                    os.remove(temp_path)
                    flash(f'Image {filename} rejected: {message}')
            else:
